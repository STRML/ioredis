...
Cluster.defaultOptions = {
  ...
  maxRedirections: 16,
  replication: false,
  replicationNodeTransform: function(node, masterOptions, otherNodes) { return node; },
...
Cluster.prototype.connect = function () {
  ...
  function readyHandler() {
    ...
    self.executeOfflineCommands();
  }

  return new Promise(function (resolve, reject) {
    if (self.status === 'connecting' || self.status === 'connect' || self.status === 'ready') {
      reject(new Error('Redis is already connecting/connected'));
      return;
    }
    self.setStatus('connecting');

    if (!Array.isArray(self.startupNodes) || self.startupNodes.length === 0) {
      throw new Error('`startupNodes` should contain at least one node.');
    }

    self.connectionPool.reset(self.startupNodes);

    var closeListener;
    var refreshListener = function () {
      self.removeListener('close', closeListener);
      self.manuallyClosing = false;
      self.setStatus('connect');
      if (self.options.enableReadyCheck) {
        self._readyCheck(function (err, fail) {
          if (err || fail) {
            // self.emit('node error', err);
    ...
    closeListener = function () {
      self.removeListener('refresh', refreshListener);
      reject(new Error('None of startup nodes is available'));
    };

    self.once('refresh', refreshListener);
    self.once('close', closeListener);
    self.once('close', self._handleCloseEvent.bind(self));

    self.refreshSlotsCache(function (err) {
      if (err && err.message === 'Failed to refresh slots cache.') {
        Redis.prototype.silentEmit.call(self, 'error', err);
        self.connectionPool.reset([]);
        return;
      }
      if (self.options.replication) {
        // In replication-only mode, make sure we have all available slaves
        // before we open a connection.
        self.selectSubscriber();
      }
    });
    if (!self.options.replication) {
      self.selectSubscriber();
...
Cluster.prototype.selectSubscriber = function () {
  var nodes = this.nodes(this.options.scaleSubscriber);
  this.subscriber = _.sample(nodes);
  if (!this.subscriber && this.options.scaleSubscriber !== 'all') {
    nodes = this.nodes('all');
    this.subscriber = _.sample(nodes);
  }
  this.emit('selectSubscriber', this.subscriber); // may be null
  if (!this.subscriber) {
    return;
  }

  // No change in subscriber; don't re-bind or resubscribe
  if (this.lastActiveSubscriber === this.subscriber && this.subscriber) {
    return;
  }

  // Re-subscribe previous channels
  var self = this;
  var lastActiveSubscriber = this.lastActiveSubscriber;
  Promise.map(['subscribe', 'psubscribe'], function (type) {
    var subscriber = lastActiveSubscriber && lastActiveSubscriber.prevCondition &&
      lastActiveSubscriber.prevCondition.subscriber;
    if (subscriber) {
      var channels = subscriber.channels(type);
      if (channels.length) {
        debug('%s %d channels', type, channels.length);
        return self.subscriber[type](channels).catch(_.noop);
      }
    }
  }).then(function () {
    self.lastActiveSubscriber = self.subscriber;
    if (self.subscriber.status === 'wait') {
      self.subscriber.connect().catch(_.noop);
    }
  });

  _.forEach(['message', 'messageBuffer'], function (event) {
    self.subscriber.on(event, function (arg1, arg2) {
      self.emit(event, arg1, arg2);
    });
  });
  _.forEach(['pmessage', 'pmessageBuffer'], function (event) {
    self.subscriber.on(event, function (arg1, arg2, arg3) {
      self.emit(event, arg1, arg2, arg3);
    });
  });
};
...
Cluster.prototype.refreshSlotsCache = function (callback) {
  ...
  // In replication mode, be sure we're only asking master nodes for info
  var pool = this.options.replication ? this.connectionPool.nodes.master : this.connectionpool.nodes.all;

  var keys = _.shuffle(Object.keys(pool));
  ...
  function tryNode(index) {
    ...
    _this.getInfoFromNode(pool[keys[index]], function (err) {
      if (_this.status === 'end') {
...
Cluster.prototype.getInfoFromNode = function (redis, callback) {
  ...
  if (this.options.replication) {
    // Non-Cluster "replication-only" mode

    // This calls 'info' which will tell us about slave nodes
    redis._readyCheck(utils.timeout(function (err, result) {
      if (err) {
        // This is actually a well-known error and unfortunately we have no other way
        // to get at the slaves if we hit it, unless we open up a new client. Leave
        // it up to the user to decide.
        if (!(/Connection in subscriber mode/i.test(err.message))) {
          redis.disconnect();
        }
        return callback(err);
      }
      ...
      // Info result includes data on slaves:
      // role:master
      // connected_slaves:2
      // slave0:ip=172.18.0.5,port=6379,state=online,offset=10494921,lag=0
      // slave1:ip=172.18.0.3,port=6379,state=online,offset=10494921,lag=0
      // ...
      //
      for (var key in result) {
        var value = result[key];
        if (key === 'role' && value !== 'master') {
          return callback(new Error('Unknown master role: ' + value));
        }
        if (/^slave\d+/.test(key)) {
          // Split the line; looks like:
          // ip=172.18.0.5,port=6379,state=online,offset=15294620,lag=0
          var slaveData = value.split(',');
          var node = { readOnly: true };
          for (var j = 0; j < slaveData.length; j++) {
            var dataSplit = slaveData[j].split('=');
            var slaveKey = dataSplit[0], slaveValue = dataSplit[1];
            if (slaveKey === 'ip') {
              slaveKey = 'host';
            }
            node[slaveKey] = slaveValue;
          }
          // Allow user to override the host et al
          if (typeof _this.options.replicationNodeTransform === 'function') {
            node = _this.options.replicationNodeTransform(node, redis.options, slaveNodes);
          }
          slaveNodes.push(node);
        }
      }

      var existingNodes = [redis.options];
      _this.connectionPool.reset(existingNodes.concat(slaveNodes));
      callback();
...
Cluster.prototype._readyCheck = function (callback) {
  if (this.options.replication) {
    var masterNodes = this.connectionPool.nodes.master;
    var master = masterNodes[Object.keys(masterNodes)[0]];
    master._readyCheck(function (err, info) {
      // Second param is expected to be empty
      return callback(err);
    });
  } else {

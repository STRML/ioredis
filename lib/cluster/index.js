...
Cluster.prototype.selectSubscriber = function () {
  ...
  if (!this.subscriber) {
    // If no subscriber is found, and we were set to slave-only, attempt
    // to find one from the 'all' pool.
    if (this.options.scaleSubscriber !== 'all') {
  ...
  if (previousChannels.subscribe.length || previousChannels.psubscribe.length) {
    ...
    _.forEach(['subscribe', 'psubscribe'], function (type) {
      ...
      if (channels.length) {
        ...
        self.subscriber[type](channels).then(function () {
          if (!--pending) {
